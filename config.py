...
class Config:
    ...
    MAIL_DEFAULT_SENDER = os.environ.get("MAIL_DEFAULT_SENDER", "noreply@example.com")  # Keep default for convenience, but recommend override

    # OTP Configuration
    OTP_ISSUER_NAME = os.environ.get("OTP_ISSUER_NAME", "PasswordManagerApp")  # Keep default for convenience

    # Session Keys
    SESSION_KEY_OTP_USER_ID = os.environ.get("SESSION_KEY_OTP_USER_ID", "otp_user_id")
    SESSION_KEY_OTP_SECRET_TEMP = os.environ.get("SESSION_KEY_OTP_SECRET_TEMP", "otp_secret_temp")

    # Template Paths
    EMAIL_LOGIN_NOTIFICATION_TEMPLATE = os.environ.get("EMAIL_LOGIN_NOTIFICATION_TEMPLATE", "email/login_notification.html")
    EMAIL_RESET_PASSWORD_TEMPLATE = os.environ.get("EMAIL_RESET_PASSWORD_TEMPLATE", "email/reset_password.html")

    # File Names
    EXPORT_CSV_FILENAME = os.environ.get("EXPORT_CSV_FILENAME", "credentials_export.csv")

    # Model Configuration
    MODEL_ENCRYPTION_SALT_LENGTH = int(os.environ.get("MODEL_ENCRYPTION_SALT_LENGTH", 16))
